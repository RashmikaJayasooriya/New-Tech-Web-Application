...
//    @Produces(MediaType.APPLICATION_JSON)
    public Response loginAction(AuthDTO authDTO, @Context HttpServletRequest request, @Context HttpServletResponse httpResponse) {
        ...
        User ud = userService.getByEmailAndPassword(authDTO.getEmail(), Encryption.encrypt(authDTO.getPassword()));
        ...
        if (ud != null) {
            ...
            if (refreshTokenCookieExistValue == null) {
                ...
                // Set cookies here
                Cookie accessTokenCookie = new Cookie("user_access_token", accessToken);
                Cookie refreshTokenCookie = new Cookie("user_refresh_token", refreshToken);
    ...
    public Response refreshToken(@FormParam("refreshToken") String refreshToken, @Context HttpServletRequest request, @Context HttpServletResponse response) {
        ...
        User userDetails = userService.getByEmailAndPassword(jwtTokenUtil.getUsernameFromToken(refreshToken), jwtTokenUtil.getPasswordFromToken(refreshToken));
